...
stop_words = {"about", "above", "after", "again", "against", "all",
                "am", "an", "and", "any", "are", "aren't", "because", "been", "before",
                "being", "below","between", "both", "but", "by", "can't", "cannot", "could",
                "couldn't", "did", "didn't", "does", "doesn't", "doing",
...
class NBDocument(object):
    ...
    def __tokenize(text):
        tokens = []
        for token in re.sub("[^\w]", " ",  text).split():
            token = token.lower()
    ...
    def fetch_tokens(self):
        with open(self.__file_path, 'r') as f:
            text = f.read()
            return NBDocument.__tokenize(text)
